...
module PositionThirdGear()
{
    ...
    PositionSecondGear() translate([ cos(Angle), sin(Angle), 0 ] * Distance2to3) children();
}
...
module PositionFourthGear()
{
    ...
    Angle =
        180 - acos((Distance2to3 ^ 2 + Distance3to4 ^ 2 - MinimalDistance2to4 ^ 2) / (2 * Distance2to3 * Distance3to4));
    PositionThirdGear() rotate([ 0, 0, Angle ]) translate([ Distance3to4, 0, 0 ]) children();
}
...
module ExitGear(AxisDiameter = IntermidiateGearAxis, Direction = Outwards, Elevation = -1, Modulus = 1)
{
    ...
    translate([ 0, 0, Elev ])
        herringbone_gear(modul = Modulus, tooth_number = ExitGearTeeth / Modulus, width = IntermidiateGearThickness,
                         bore = AxisDiameter, pressure_angle = 20, helix_angle = -20 * Direction, optimized = false);
    difference()
    {
...
module MotorGear()
{
    ...
    translate([ 0, 0, 1.5 * InterGearGap ])
        herringbone_gear(modul = MotorGearModulus, tooth_number = MotorGearTeeth, width = 6, bore = MotorAxisDiameter,
                         pressure_angle = 20, helix_angle = Inwards * -20, optimized = false);
    difference()
    {
        union()
        {
            cylinder(d = (MotorGearTeeth + 2) * MotorGearModulus, h = 1.5 * InterGearGap);
            cylinder(d = SensorDiscDiameter, h = InterGearGap / 2);
        }
        // Eixo
        translate([ 0, 0, -InterGearGap / 2 ]) cylinder(d = MotorAxisDiameter, h = 3 * InterGearGap);
        // Furos do sensor
        for (i = [0:Holes - 1])
        {
...
module FourthGear()
{
    ...
    intersection()
    {
        herringbone_gear(modul = MotorGearModulus, tooth_number = FourthGearTeeth, width = MotorGearThickness,
                         bore = Axis, pressure_angle = 20, helix_angle = 20 * Outwards, optimized = false);
        union()
        {
            translate(v = [ 0, 0, 1 ])
                cylinder(h = MotorGearThickness - 1, d = FourthGearTeeth * MotorGearModulus + 1, center = false);
            cylinder(h = 1, d1 = FourthGearTeeth * MotorGearModulus - 1, d2 = FourthGearTeeth * MotorGearModulus + 1,
                     center = false);
        }
    }
...
module ThirdGear()
{
    ...
    herringbone_gear(modul = 1, tooth_number = ThirdGearTeeth, width = IntermidiateGearThickness, bore = Axis,
                     pressure_angle = 20, helix_angle = 20 * Inwards, optimized = true);
...
module SecondGear()
{
    ...
    // Ângulo de hélice para saida é -20 e entrada 20
    // Esta é uma engranagem de entrada
    herringbone_gear(modul = 1, tooth_number = SecondGearTeeth, width = IntermidiateGearThickness, bore = Axis,
                     pressure_angle = 20, helix_angle = 20 * Outwards, optimized = true);
...
module SpoolGear()
{
    translate([ 0, 0, HubHeigth / 2 ])
    {
            ...
            union()
            {
                ...
                difference()
                {
                    union()
                    {
                        // Chanfro de encaixe
                        translate([
                            0, 0, (SpoolWidth + 2 * PartsMinThickness + IntermidiateGearThickness) / 2 -
                            PartsMinThickness
                        ]) cylinder(d1 = SpoolInternalDiameter - 2 * Gap,
                                    d2 = SpoolInternalDiameter - PartsMinThickness, h = PartsMinThickness);
                        // Cilindro central
                        translate([ 0, 0, (PartsMinThickness + IntermidiateGearThickness / 2) - PartsMinThickness / 2 ])
                            cylinder(d = SpoolInternalDiameter - 2 * Gap, h = SpoolWidth - PartsMinThickness,
                                     center = true);
                        // Base
                        translate([ 0, 0, -SpoolWidth / 2 ])
                            cylinder(d = SpoolInternalDiameter + 3 * PartsMinThickness,
                                     h = 2 * PartsMinThickness + IntermidiateGearThickness, center = true);
                    }
                ...
                for (i = [0:3])
                {
                    rotate([ 0, 0, i * 45 ])
                    {
                        ...
                        cube([ SpoolInternalDiameter - PartsMinThickness - Gap, PartsMinThickness - Gap, HubHeigth ],
                             center = true);
                        // Chanfros de encaixe
                        for (i = [ 0, 1 ])
                        {
                            rotate([ 0, 0, i * 180 ]) translate([
                                ...
                                (SpoolWidth + 2 * PartsMinThickness + IntermidiateGearThickness) / 2 - 2 *
                                PartsMinThickness
        ...
        // Esta é uma engranagem de entrada
        translate([ 0, 0, -HubHeigth / 2 ])
            herringbone_gear(modul = FirstGearModulus, tooth_number = FirstGearTeeth / FirstGearModulus,
                             width = IntermidiateGearThickness, bore = SpoolInternalDiameter + 2 * PartsMinThickness,
...
module Assembly()
{
    ...
    module Head()
    {
        ...
        {
            Spool();
            translate([ 0, 0, SpoolUsefulWidth + 1.5 * PartsMinThickness ]) Spool(true);
        }
    ...
    PositionSecondGear() translate([ 0, 0, -IntermidiateGearThickness - InterGearGap ])
        // color("red")
        % SecondGear();
    PositionThirdGear() translate([ 0, 0, IntermidiateGearThickness ]) rotate([ 180, 0, 0 ]) color("green") ThirdGear();
    PositionFourthGear() translate([ 0, 0, -IntermidiateGearThickness - 2 * InterGearGap - MotorGearThickness ])
