...
// A largura útil do carretel é calculada de acordo com o tamanho do parafuso
// eixo
SpoolUsefulWidth = 30;
...
FirstGearTeeth =
    floor((SpoolExternalDiameter + NozzleHeight) / FirstGearModulus) *
    FirstGearModulus;
SecondGearTeeth =
    floor(FirstGearTeeth - (Bearing608ExternalDiamenter + PartsMinThickness -
                            ExitGearTeeth / 2));
ThirdGearTeeth = floor(SecondGearTeeth - ExitGearTeeth / 2);
FourthGearTeeth =
    floor((ThirdGearTeeth - ExitGearTeeth / 2) / MotorGearModulus);
...
module PositionFirstGear() { translate([ 0, 0, 0 ]) children(); }

module PositionSecondGear() {
  // A segunda engrenagem é posicionada de modo que sua parte inferior fique à
  // mesma altura da engrenagem 1
  Angle = 180 + asin((FirstGearTeeth - SecondGearTeeth) / (2 * Distance1to2));
  PositionFirstGear() {
    translate([ cos(Angle), sin(Angle), 0 ] * Distance1to2) children();
  }
}

module PositionThirdGear() {
  MinimalDistance1to3 = (FirstGearTeeth + ThirdGearTeeth + 6) / 2;
  Angle = 180; // acos((Distance1to2^2+Distance2to3^2-MinimalDistance1to3^2)/(2
               // * Distance1to2 * Distance2to3));
  PositionSecondGear() translate([ cos(Angle), sin(Angle), 0 ] * Distance2to3)
      children();
}
...
module PositionFourthGear() {
  ...
  Angle = 180 -
          acos((Distance2to3 ^ 2 + Distance3to4 ^ 2 - MinimalDistance2to4 ^ 2) /
               (2 * Distance2to3 * Distance3to4));
  PositionThirdGear() rotate([ 0, 0, Angle ]) translate([ Distance3to4, 0, 0 ])
      children();
}

module ExitGear(AxisDiameter = IntermidiateGearAxis, Direction = Outwards,
                Elevation = -1, Modulus = 1) {
  Elev =
      (Elevation == -1) ? InterGearGap + IntermidiateGearThickness : Elevation;
  translate([ 0, 0, Elev ]) herringbone_gear(
      modul = Modulus, tooth_number = ExitGearTeeth / Modulus,
      width = IntermidiateGearThickness, bore = AxisDiameter,
      pressure_angle = 20, helix_angle = -20 * Direction, optimized = false);
  difference() {
    cylinder(d = ExitGearTeeth + 2, h = Elev);
    translate([ 0, 0, -.5 ]) cylinder(d = AxisDiameter, h = Elev + 1);
  }
}
...
module MotorGear() {
  ...
  SensorDiscDiameter =
      MotorDiameter + 2 * IRSensorDiameter + 3 * PartsMinThickness;
  SensorHolesRadius =
      MotorDiameter / 2 + PartsMinThickness + IRSensorDiameter / 2;
  ...
  translate([ 0, 0, 1.5 * InterGearGap ])
      herringbone_gear(modul = MotorGearModulus, tooth_number = MotorGearTeeth,
                       width = 6, bore = MotorAxisDiameter, pressure_angle = 20,
                       helix_angle = Inwards * -20, optimized = false);
  difference() {
    union() {
      cylinder(d = (MotorGearTeeth + 2) * MotorGearModulus,
               h = 1.5 * InterGearGap);
      cylinder(d = SensorDiscDiameter, h = InterGearGap / 2);
    }
    // Eixo
    translate([ 0, 0, -InterGearGap / 2 ])
        cylinder(d = MotorAxisDiameter, h = 3 * InterGearGap);
    // Furos do sensor
    for (i = [0:Holes - 1]) {
      rotate([ 0, 0, i * Angle ]) translate([ SensorHolesRadius, 0, 0 ])
...
module FourthGear() {
  ...
  // Esta é a parte do miolo que contem os furos para o sensor óptico para
  // controle do motor
  translate([ 0, 0, MotorGearThickness / 4 ]) {
    difference() {
      cylinder(d = CoreDiameter, h = MotorGearThickness / 2, center = true);
      cylinder(d = IntermidiateGearAxis, h = MotorGearThickness / 2 + 1,
               center = true);
      // Furos para o sensor óptico
      for (i = [0:NumberOfHoles - 1]) {
        rotate([ 0, 0, i * HoleAngle ]) translate([ HoleRadius, 0, 0 ])
            cylinder(d = LdrDiameter, h = MotorGearThickness / 2 + 1,
                     center = true);
      }
    }
  }
  ...
  herringbone_gear(modul = MotorGearModulus, tooth_number = FourthGearTeeth,
                   width = MotorGearThickness, bore = Axis, pressure_angle = 20,
                   helix_angle = 20 * Outwards, optimized = false);
  ExitGear(Direction = Outwards, Elevation = MotorGearThickness +
                                             IntermidiateGearThickness +
                                             2 * InterGearGap);
}
...
module ThirdGear() {
  ...
  herringbone_gear(modul = 1, tooth_number = ThirdGearTeeth,
                   width = IntermidiateGearThickness, bore = Axis,
                   pressure_angle = 20, helix_angle = 20 * Inwards,
                   optimized = true);
...
module SecondGear() {
  ...
  // Ângulo de hélice para saida é -20 e entrada 20
  // Esta é uma engranagem de entrada
  herringbone_gear(modul = 1, tooth_number = SecondGearTeeth,
                   width = IntermidiateGearThickness, bore = Axis,
                   pressure_angle = 20, helix_angle = 20 * Outwards,
                   optimized = true);
  ExitGear(AxisDiameter = Axis, Direction = Outwards,
           Modulus = FirstGearModulus);
}
...
module Spool(FenseOnly = false) {
  module Fense() {
    ...
    HoleDiameter = (SpoolFenseDiameter - SpoolExternalDiameter) / 2 -
                   2 * PartsMinThickness;
    // Calculando quantos furos cabem na curcunferência média deixando espessura
    // mínima
...
module SpoolGear() {
  translate([ 0, 0, HubHeigth / 2 ]) {
    difference() {
      union() {
        ...
        difference() {
          union() {
            ...
            translate([
              0, 0,
              (SpoolWidth + 2 * PartsMinThickness + IntermidiateGearThickness) /
                      2 -
              PartsMinThickness
            ...
            translate([
              0, 0,
              (PartsMinThickness + IntermidiateGearThickness / 2) -
                  PartsMinThickness / 2
            ...
            translate([ 0, 0, -SpoolWidth / 2 ])
                cylinder(d = SpoolInternalDiameter + 3 * PartsMinThickness,
                         h = 2 * PartsMinThickness + IntermidiateGearThickness,
                         center = true);
          }
          cylinder(d = SpoolInternalDiameter - 2 * PartsMinThickness,
                   h = HubHeigth + 1, center = true);
        }
        // Aletas de fixação
        for (i = [0:3]) {
          rotate([ 0, 0, i * 45 ]) {
            // Aletas principais de encaixe
            translate([ 0, 0, -PartsMinThickness ]) cube(
                [
            ...
            for (i = [ 0, 1 ]) {
              rotate([ 0, 0, i * 180 ]) translate([
                (SpoolInternalDiameter - Gap) / 2, 0,
                (SpoolWidth + 2 * PartsMinThickness +
                 IntermidiateGearThickness) /
                        2 -
                    2 *
                PartsMinThickness
      ...
      translate([ 0, 0, -HubHeigth / 2 - 1 ]) cylinder(
          d = Bearing608ExternalDiamenter + 2 * Gap, h = Bearing608Width + 1);
      // O furo para o rolamento superior deve conter a cabeça do parafuso usado
      // como eixo
    ...
    // Ângulo de hélice para saida é -20 e entrada 20
    // Esta é uma engranagem de entrada
    translate([ 0, 0, -HubHeigth / 2 ]) herringbone_gear(
        modul = FirstGearModulus,
        tooth_number = FirstGearTeeth / FirstGearModulus,
        width = IntermidiateGearThickness,
        bore = SpoolInternalDiameter + 2 * PartsMinThickness,
...
module SpoolGearAxis() {
  ...
  translate([ 0, 0, AxisHeigth / 2 ]) difference() {
    cylinder(d = Bearing608MeanDiamenter - 2 * Gap, h = AxisHeigth,
             center = true);
    cylinder(d = M8SwcreewDiameter + 2 * Gap, h = AxisHeigth + 1,
             center = true);
  }
}
...
module Assembly() {
  ...
  module Head() {
    ...
    translate([ 0, 0, IntermidiateGearThickness + 2 * PartsMinThickness ])
        ...
        % CompleteSpool();
    SpoolGear();
    translate([ 0, 0, HubHeigth - M8SwcreewLength ]) color("black") M8Swcreew();
  }
  PositionFirstGear() Head();
  PositionSecondGear()
          translate([ 0, 0, -IntermidiateGearThickness - InterGearGap ])
      // color("red")
      % SecondGear();
  PositionThirdGear() translate([ 0, 0, IntermidiateGearThickness ])
      rotate([ 180, 0, 0 ]) color("green") ThirdGear();
  PositionFourthGear() translate([
    0, 0, -IntermidiateGearThickness - 2 * InterGearGap -
    MotorGearThickness
  ]) color("gray") FourthGear();
}
...
// h=IntermidiateGearThickness+3*InterGearGap + MotorGearThickness);

// Espaçador para a segunda e terceira engrenagens
// Spacer(dIn=IntermidiateGearAxis+2*Gap, dOut = ExitGearTeeth - 2*Gap,
// h=MotorGearThickness+2*InterGearGap);
...
// Spacer(dIn=3.5, dOut=10,
// h=MotorGearThickness+2*IntermidiateGearThickness+4*InterGearGap);
