...
// Altura do carrier
CarrierHeight = PlanetaryGearThickness + 1.5 * InterGearGap + 2 * PartsMinThickness;
...
Za = ceil((SunGearBore + 2 * PartsMinThickness + 2.5 * PlanetaryGearModulus) / PlanetaryGearModulus);

// (E)stimando o número mínimo de dentes da engrenagem planeta
ZbE = ceil(((SpoolExternalDiameter + 2 * PartsMinThickness - Za * PlanetaryGearModulus)) / PlanetaryGearModulus);
...
function FindZc(Zc0, Za0) = ((((Zc0 - Za0) % 2 == 0) && ((Zc0 + Za0) % NumberOfPlanets == 0)) || (Zc0 > 1000))
                                ? Zc0
                                : FindZc(Zc0 + 1, Za0);
...
module PositionLookScrews()
{
    ...
    for (i = [0:NumberOfPlanets])
    {
        rotate([ 0, 0, i * Angle + HalfAngle ]) translate([ (DZa + DZb) / 2 + PartsMinThickness, 0, 0 ]) children();
    }
}
...
module GearSpacer(bore = 3, gap = Gap)
{
    difference()
    {
        cylinder(h = InterGearGap / 2 - gap / 2, d = bore + 2 * PartsMinThickness, center = false);
        translate(v = [ 0, 0, -InterGearGap / 4 ]) cylinder(h = InterGearGap, d = bore, center = false);
    }
}

module SunGear()
{
    rotate([ 0, 0, 0 ]) spur_gear(modul = PlanetaryGearModulus, tooth_number = Za, width = PlanetaryGearThickness,
                                  bore = SunGearBore, pressure_angle = 20, helix_angle = 0, optimized = false);
    translate(v = [ 0, 0, PlanetaryGearThickness ]) GearSpacer(bore = SunGearBore);
}

module PlanetGear()
{
    spur_gear(modul = PlanetaryGearModulus, tooth_number = Zb, width = PlanetaryGearThickness,
              bore = IntermidiateGearBushing, pressure_angle = 20, helix_angle = 0, optimized = false);
    translate(v = [ 0, 0, PlanetaryGearThickness ]) GearSpacer(bore = IntermidiateGearBushing);
}

module RingGear()
{
    ring_gear(modul = PlanetaryGearModulus, tooth_number = Zc, width = NumberOfCarries * CarrierHeight,
              rim_width = PartsMinThickness, pressure_angle = 20, helix_angle = 0);
}
...
module CarrierBasePlate(h = PartsMinThickness)
{
    difference()
    {
        cylinder(h = h, r = (DZa + DZb + IntermidiateGearAxis) / 2 + 2 * PartsMinThickness, center = false);
        // Furos dos eixos das engrenagens planetas
        translate(v = [ 0, 0, -.5 ]) PositionPlanetGear() cylinder(h = h + 1, d = IntermidiateGearAxis);
...
module CarrierLock(h = CarrierLockHeight)
{
    ...
    PositionLookScrews() rotate_extrude(angle = 360, convexity = 2)
        translate(v = [ FixingScrewHeadDiamenter / 2 + h, 0, 0 ]) rotate([ 0, 0, 45 ]) square(Side, center = true);
}
...
module CarrierLockScrew(head = true, lenght = 3)
{
    PositionLookScrews() union()
    {
        if (head)
        {
            translate(v = [ 0, 0, -.5 ]) cylinder(h = lenght, d = FixingScrewPassThruDiamenter + 1, center = false);
            translate(v = [ 0, 0, lenght - FixingScrewHeadHeight ])
                cylinder(h = FixingScrewHeadHeight + 1, d = FixingScrewHeadDiamenter);
        }
        else
        {
            translate(v = [ 0, 0, -.5 ]) cylinder(h = lenght + 1, d = FixingScrewScrewingDiamenter, center = false);
        }
    }
}
    ...
    difference()
    {
        ...
        union()
        {
            difference()
            {
                ...
                translate(v = [ 0, 0, PartsMinThickness ]) PositionPlanetGear() cylinder(
                    h = PlanetaryGearThickness + InterGearGap + 1, d = DZb + 2 * PlanetaryGearModulus + 4 * Gap + 2);
            }
            // Espaçadores das engrenagens
            translate(v = [ 0, 0, PartsMinThickness ]) PositionPlanetGear() GearSpacer(bore = IntermidiateGearAxis);
            // Travas de alinhamento
            translate(v = [ 0, 0, PlateThickness ]) CarrierLock(h = CarrierLockHeight);
        }
        // Furo da engrenagem sol
        translate(v = [ 0, 0, -.5 ]) cylinder(h = PartsMinThickness + PlanetaryGearThickness + InterGearGap + 1,
...
module UpperCarrier()
{
    ...
    difference()
    {
        ...
        union()
        {
            ...
            cylinder(h = 2 * PartsMinThickness + 1.5 * InterGearGap, d = DZa + 2 * PlanetaryGearModulus,
                     center = false);
            // Espaçador entre os estágios
            translate(v = [ 0, 0, PartsMinThickness ])
                GearSpacer(bore = DZa + 2 * (PlanetaryGearModulus + PartsMinThickness), gap = 0);
        }
        // Furo do eixo principal
        translate(v = [ 0, 0, -.5 ])
            cylinder(h = 2 * PartsMinThickness + 1.5 * InterGearGap + 1, d = SunGearBore, center = false);
    }
    // Engrenagem sol
...
module Spool(FenseOnly = false)
{
    ...
    module Fixtures()
    {
        ...
        for (i = [0:3])
        {
            rotate([ 0, 0, i * 90 + 11.75 ]) translate([ SpoolInternalDiameter / 2 + PartsMinThickness, 0, 0 ])
...
module Aletas(heigth = SpoolWidth, cavar = false)
{
    ...
    else
    {
        for (i = [0:3])
        {
            rotate([ 0, 0, i * 45 ])
            {
                ...
                translate([ 0, 0, (heigth - 2 * FittingChanferHeight) / 2 ]) cube(
                    [
                        SpoolInternalDiameter + 2 * (PartsMinThickness - Gap), PartsMinThickness - Gap, heigth - 2 *
                        FittingChanferHeight
                    ],
                    center = true);
                // Aletas internas de suporte
                translate([ 0, 0, heigth - FittingChanferHeight ]) cube(
                    [
                        SpoolInternalDiameter - PartsMinThickness - Gap, PartsMinThickness - Gap, 2 *
                        FittingChanferHeight
                    ],
                    center = true);
                // Chanfros de encaixe
                ...
                {
                    rotate([ 0, 0, i * 180 ])
                        translate([ (SpoolInternalDiameter - Gap) / 2, 0, heigth - 2 * FittingChanferHeight ])
                            scale([ (2 * PartsMinThickness - Gap) / (PartsMinThickness - Gap), 1, 1 ])
                                rotate([ 0, 0, 45 ]) cylinder(d1 = sqrt(2) * (PartsMinThickness - Gap), d2 = 0,
    ...
    union()
    {
        difference()
        {
            union()
            {
                        ...
                        translate(v = [ 0, 0, HubBaseHeigth ])
                        {
                            ...
                            translate([ 0, 0, SpoolWidth - FittingChanferHeight ])
                                cylinder(d1 = SpoolInternalDiameter - 2 * Gap,
                                         d2 = SpoolInternalDiameter - FittingChanferHeight, h = FittingChanferHeight);
                            // Cilindro central

                            cylinder(d = SpoolInternalDiameter - 2 * Gap, h = SpoolWidth - FittingChanferHeight);
                        }
                        // Base
                        cylinder(d = SpoolInternalDiameter + 3 * PartsMinThickness, h = HubBaseHeigth, center = false);
                    }
                    // Furo central
                    translate(v = [ 0, 0, -.5 ])
                        cylinder(d = SpoolInternalDiameter - 2 * PartsMinThickness, h = HubHeigth + 1);
                }
                // Aletas de fixação
                for (i = [0:3])
                {
                    ...
                    {
                        // Aletas principais de encaixe
                        translate([ 0, 0, (HubHeigth - 2 * FittingChanferHeight) / 2 ]) cube(
                            [
                                SpoolInternalDiameter + 2 * (PartsMinThickness - Gap), PartsMinThickness - Gap,
                                HubHeigth - 2 *
                                FittingChanferHeight
                            ],
                            center = true);
                        ...
                        for (i = [ 0, 1 ])
                        {
                            rotate([ 0, 0, i * 180 ]) translate(
                                [ (SpoolInternalDiameter - Gap) / 2, 0, HubHeigth - 2 * FittingChanferHeight ])
                                scale([ (2 * PartsMinThickness - Gap) / (PartsMinThickness - Gap), 1, 1 ])
                                    rotate([ 0, 0, 45 ]) cylinder(d1 = sqrt(2) * (PartsMinThickness - Gap), d2 = 0,
                                                                  h = FittingChanferHeight, $fn = 4);
                        }
                ...
                // Eixo central
                cylinder(d = Bearing608ExternalDiamenter + 2 * PartsMinThickness, h = HubHeigth);
            }
            // Furo do eixo central
            translate([ 0, 0, -.5 ]) cylinder(d = Bearing608MeanDiamenter + 2 * Gap, h = HubHeigth + 1);
            // Furos para os rolamentos
            translate([ 0, 0, -1 ]) cylinder(d = Bearing608ExternalDiamenter + 2 * Gap, h = Bearing608Width + 1);
...
module MotorHub()
{
    union()
    {
        ...
        difference()
        {
            UpperCarrierBasePlate(h = PartsMinThickness);
            translate(v = [ 0, 0, -.5 ])
                cylinder(h = Bearing608Width + 1, d = Bearing608ExternalDiamenter + 2 * Gap, center = false);
        }
    }
}
...
module GearBoxCase()
{
    ...
    difference()
    {
        cylinder(h = CaseHeight, d1 = CaseDiameter, d2 = CaseDiameter + 2 + PartsMinThickness, center = false);
        translate(v = [ 0, 0, -.5 ])
            cylinder(h = CaseHeight + 1, d = CaseDiameter - 2 * PartsMinThickness, center = false);
    }
    // Flange e tampo
    translate(v = [ 0, 0, CaseHeight ]) difference()
    {
        ...
        union()
        {
            translate(v = [ 0, 0, PartsMinThickness / 2 ])
                cylinder(h = PartsMinThickness / 2, d1 = CaseDiameter + 2 * CaseFlangeWidth,
                         d2 = CaseDiameter + 2 * CaseFlangeWidth - PartsMinThickness, center = false);
            cylinder(h = PartsMinThickness / 2, d = CaseDiameter + 2 * CaseFlangeWidth, center = false);
            // Espaçador
            translate(v = [ 0, 0, -InterGearGap / 2 ]) GearSpacer(bore = HubHole, gap = 0);
        }
        // Furo do hub
        translate(v = [ 0, 0, -.5 - InterGearGap ])
            cylinder(h = PartsMinThickness + InterGearGap + 1, d = HubHole, center = false);
        // Parafusos de fixação

        for (i = [0:NumberOfScrews - 1])
        {
            rotate([ 0, 0, i * 360 / NumberOfScrews ]) translate(v = [ (CaseDiameter + CaseFlangeWidth) / 2, 0, 0 ])
            {
                translate([ 0, 0, PartsMinThickness - CaseFixingScrewHeadResses ])
                    cylinder(h = CaseFixingScrewHeadResses + 1, d = CaseFixingScrewHeadDiamenter);
                translate(v = [ 0, 0, -.5 ])
                    cylinder(h = PartsMinThickness + 1, d = CaseFixingScrewDiamenter, center = false);
            }
        }
    }
    RingGear();
